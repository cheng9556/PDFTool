...
Page({
  ...
  convertToExcel() {
    ...
    this.setData({ converting: true, error: '', excelUrl: '' });

    wx.uploadFile({
      url: `${SERVER_URL}/pdf/toexcel`,
  ...
  downloadExcel() {
    if (!this.data.excelUrl) return;

    wx.downloadFile({
      url: this.data.excelUrl,
      success: (res) => {
        if (res.statusCode === 200) {
          wx.openDocument({
            ...
            fail: (err) => {
              ...
              wx.showToast({ title: '无法打开Excel文件', icon: 'none' });
            }
          });
        }
      },
