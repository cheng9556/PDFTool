...
#### 19. 文件转长图 ✨新增
- **接口**: `POST /file/to-long-image`
- **功能**: 将PDF/Word/Excel/PPT转换为长图
- **特性**:
  ...
  - 建议20页以内，最多50页
  - 可调DPI（影响质量和文件大小）
- **使用库**: `PyMuPDF`, `PIL`, `requests`（调用Java服务转换Office文件）
- **前端页面**: `pages/longimage/index`

#### 20. 长图打印 ✨新增
- **接口**: `POST /longimage/print`
- **功能**: 将长图裁剪并排版到A4页面中，适合打印
- **特性**:
  - 支持图片和PDF文件输入
  - 三种分栏布局（单列/两列/三列）
  - 三种质量选项（150/300/600 DPI）
  - 自动计算裁剪和排版
  - A4标准尺寸（595x842点）
  - 智能边距和间距
  - 高质量输出，适合打印
- **使用库**: `PyMuPDF`, `PIL`
- **前端页面**: `pages/printimage/index`
- **技术细节**:
  - 使用Pillow进行图片裁剪和缩放
  - 使用PyMuPDF创建A4页面并插入图片
  - 自动计算每列宽度和每段高度
  - 保持图片宽高比
  - 支持多列布局以节省纸张
